...
def main():
    ...
    for acctId, cred in rolesCred.items():
        ...
        # NOTE: _info/_warn are already imported at module scope (top of file).
        # A redundant local import here made Python treat them as function-local
        # variables, causing UnboundLocalError on earlier uses inside main().
...
if __name__ == '__main__':
    # freeze_support() is required for Windows (spawn start method) so that
    # worker processes re-importing this module do not re-execute the main
    # program body. It is a no-op on Linux/CloudShell (fork), keeping this
    # single file compatible with both environments.
